...
CURRENT_TOR_VERSION=$(cat tor_download_page | grep 'tor-.*tar.gz"' | sed -E 's/^.*href="tor-([0-9\.]+)\.tar\.gz".*$/\1/g' | grep -E '^[0-9\.]+$' | sort -V | tail -1)
...
# gpg -vv --auto-key-locate nodefault,wkd --locate-keys arma#torproject.org
...
DONE=0
while [ $DONE -eq 0 ]; do
        curl -fLO ${CURRENT_TOR_VERSION_URL} && tar xf ${CURRENT_TOR_VERSION_FILENAME} --strip-components 1 && gpg --verify ${CURRENT_TOR_VERSION_FILENAME}.asc ${CURRENT_TOR_VERSION_FILENAME}
        [ $? -eq 0 ] && DONE=1 || {
